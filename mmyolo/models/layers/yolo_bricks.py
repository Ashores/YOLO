...
from mmyolo.models.DSA import Dynamicsparsity
import numpy as np
import torch
...
from mmyolo.models.ops_dcnv3.modules import DCNv3

class DCNV3_YoLo(nn.Module):
    def __init__(self, inc, ouc, k, padding, g,
                 conv_cfg=None, norm_cfg=None, act_cfg=None):
        super().__init__()

        self.conv = ConvModule(inc, ouc, k, padding=padding, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.dcnv3 = DCNv3(ouc, kernel_size=k, stride=1, group=g)
...
class ELANBlock(BaseModule):
    ...
    def __init__(self,
        ...
        incha=[64, 128, 256, 384, 512]
        g = [1, 2, 4, 6, 8]
        index = incha.index(middle_channels)
...
class SPPFCSPBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.final_conv = ConvModule(
            ...
            norm_cfg=norm_cfg,
            act_cfg=act_cfg)
        self.attn = Dynamicsparsity(out_channels)
    ...
    def forward(self, x) -> Tensor:
        ...
        return self.attn(self.final_conv(torch.cat((x1, x2), dim=1)))
