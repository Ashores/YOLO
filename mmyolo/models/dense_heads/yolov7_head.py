...
class YOLOv7HeadModule(YOLOv5HeadModule):
    ...
    def init_weights(self):
        ...
        for mi, s in zip(self.convs_pred, self.featmap_strides):  # from
            ...
            b.data[:, 5:] += math.log(0.6 / (1.5 - 0.99))
...
class YOLOv7p6HeadModule(YOLOv5HeadModule):
    ...
    def _init_layers(self):
        ...
        self.object_conv = nn.ModuleList()
    ...
    def init_weights(self):
        ...
        for conv in self.cls_preds:
            conv.bias.data.fill_(0.02)
            conv.weight.data.fill_(-0.01)

        for conv in self.reg_preds:
            conv.bias.data.fill_(0.03)
            conv.weight.data.fill_(0.01)

        for conv in self.obj:
            conv.bias.data.fill_(0.01)
            conv.weight.data.fill_(0.01)
        # for a,c,aux, s in zip(self.cls_preds,self.obj, self.aux_convs_pred, self.featmap_strides):  # from
        #     if self.use_aux:
        #         aux = aux[1]  # nn.Conv2d
        #         b = aux.bias.data.view(3, -1)
        #         # obj (8 objects per 640 image)
        #         b.data[:, 4] += math.log(8 / (640 / s)**2)
        #         b.data[:, 5:] += math.log(0.6 / (80 - 0.99))
        #         a.bias.data = b.data[:, 4:5].view(-1)
        #         c.bias.data = b.data[:, 5:].view(-1)
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        return multi_apply(self.forward_single, x, self.stems, self.cls_convs,
                           self.cls_preds, self.object_conv, self.reg_convs, self.reg_preds, self.obj, self.aux_convs_pred)

    def forward_single(self, x: Tensor, stem: nn.Module, cls_conv: nn.Module,
                       cls_pred: nn.Module, obj_conv:nn.Module, reg_conv: nn.Module, reg_pred: nn.Module, obj: nn.Module,
        ...
        cls_score = cls_pred(cls_feat)#2
        bbox_pred = reg_pred(reg_feat)
        objectness = obj(obj_feat)#3
